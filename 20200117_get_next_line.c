...
/*   Updated: 2020/01/15 15:23:34 by nhariman      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
int		fill_buf(char **line, char *restbuf, char *buf, int bytes_read)
{
	int		n_position;
	...
	*line = ft_strdup("");
	if (!restbuf)
	{
		while (bytes_read > 0)
		{
			...
			buf[bytes_read] = '\0';
			if (fill_buf(*line, restbuf, buf, bytes_read) < 1)
				
			// scan for newline (if no newline, put in *line and repeat, if newline, put remainder in resbuf, return 1)
			// if EOF && restbuf is empty, return 0.
		}
	}
}
